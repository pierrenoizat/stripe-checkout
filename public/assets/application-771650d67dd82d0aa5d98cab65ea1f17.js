...
var linkUrl = "http://localhost:3000/orders/" + orderId;
...
function myTimer() {
	...
	if ((Number(d)/1000 < maxTime) && (paid == false)) {
		...
		var url = "http://localhost:3000/orders/" + orderId; // calls show action in orders controller to fetch the status attribute
...
function myStopFunction() {
	if (paid == true) {
		...
		if (signedin == true) {
			...
			element = document.getElementById("bitcoin-card");
		...
		var element = document.getElementById("bitcoin-card");
		if (element != null)
			{
				element.innerHTML = "Pay with Card";
			}
